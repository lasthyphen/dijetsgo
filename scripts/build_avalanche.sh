...
if [ -z ${ROCKSDBALLOWED+x} ]; then
    ...
    go build -ldflags "-X github.com/lasthyphen/dijetsgo/version.GitCommit=$git_commit $static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
else
    echo "Building AvalancheGo with rocksdb enabled..."
    go build -tags rocksdballowed -ldflags "-X github.com/lasthyphen/dijetsgo/version.GitCommit=$git_commit $static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
fi
